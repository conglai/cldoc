...
module.exports = function factoryDoc(
  ...
  function renderTagTpl(block, data) {
    ...
    let htmlStr;
    try{
      htmlStr = pug.renderFile(`${MODEL_DIR}/${tplName}`, tplData);
    } catch (e) {
      return _errorTag(block, e);
    }
    return htmlStr;
  ...
  //## 渲染代码块
  function renderCode(block, navTree, data) {
    switch (block.codeType) {
      ...
      case 'json-table':
        return renderJSONTable(block, data);
      default:
        return _mdCodeTag(block);
    }
  }
  ...
  function renderBlocks(navItem, navTree) {
    ...
    for (let i = 0, l = blocks.length; i < l ; i++) {
      let block = blocks[i];
      if(block.type === 'code') {
        mdStr += renderCode(block, navTree, { url, title, keywords, basename });
      } else {
        mdStr += block.content;
      }
    }
