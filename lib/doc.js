...
module.exports = function factoryDoc(
  ...
  // ## 渲染json-table
  function renderJSONTable(codeStr) {
    codeStr = utils.filterComment(codeStr);
    let data = JSON.parse(codeStr);
    let titleRow = data.shift();
    let keys = Object.keys(titleRow);
    return pug.renderFile(`${TPL_DIR}/json-table.pug`, {
      keys: keys,
      titleRow: titleRow,
      rows: data
    });
  ...
  function renderCode(block, navTree) {
    switch (block.codeType) {
      ...
      case 'json-table':
        let result = false;
        try {
          result = renderJSONTable(block.content);
        } catch (e) {
          block.codeType = 'js';
          let msgs = e.stack.split('\n');
          block.content += `\n${msgs[0]}\n${msgs[1]}`;
        }
        if(result) {
          return result;
